...
- **Group pages (Oki, 25 Sep 2026):** near-identical ETFs share ONE report, led by the largest fund (file `reports/etf/<lead ticker>_analysis.html`; hero, charts and metrics are the lead's). Near-identical = same index or same universe/methodology, same asset class, same listing currency, same hedging, holdings overlap ≥ ~90% by weight (Σ min(w_lead, w_member), computed from holdings files dated ≤ banner; else "same index" stated as the basis). CAD- vs USD-listed, hedged vs unhedged = different groups.
- **Owner rulings on grouping (Oki, 25 Sep 2026):** (1) **overlap decides** — ≥90% holdings overlap by weight, matched at security level (ISIN/SEDOL, or exchange ticker + name/country check when files lack identifiers; share classes and DR lines of one company combined; stock lines only, rescaled to 100%), same currency and hedging = equivalent, whatever the index family (e.g. VV with VOO, VEU with VXUS, SPTM with VTI; SPDW at 87.8% strict is a peer of VEA). Never use name-prefix matching (it misses renamed companies). (2) **Swap-based funds on the same index and currency are equivalents** (HXS, HXT, HXCN) — overlap cell "same index (swap)", with a note on structure. (3) **Asset-allocation one-ticket portfolios** of the same type (all-equity, CAD) share one page: XEQT with VEQT, ZEQT, HEQT, overlap stated at the level it can be computed.
- **No-member wording:** when no fund qualifies, the block says "No fund among those checked holds a near-identical portfolio (≥90% overlap by weight)." and lists the closest funds with their overlap and method.
- **Company names:** use current names (issuer files of other providers, company sites), never stale labels in an issuer's holdings file (e.g. "Facebook Inc.", "General Electric" for GE Aerospace).
- **"Equivalent ETFs" block** directly after the hero, before 01: a compact table — ticker, fund name, issuer, index, listing/currency, fee (expense ratio or MER, with document date), net assets (date), overlap with the lead (value + method, or "same index"). One line above it: "These funds hold near-identical portfolios; the analysis below uses <lead> and applies to all of them unless noted." Differences that matter (fee, securities lending, structure, distribution frequency, inception) go in a one-line note per member, not a verdict. Section 05 peers then cover the next-closest *different* funds, not the equivalents.
