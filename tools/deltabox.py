...
Only reports with a genuine prior edition get a box. Every figure below is taken from the report's own
prose — nothing is newly researched here, it is existing analysis given a consistent structure.
...
class DeltaBox(NamedTuple):
    """One refreshed report's "what changed" box: the previous edition's date and price, this edition's, the box
    state (a TAGS key) and its text."""
...
BOXES = {  # slug -> its box: the September 2026 refresh editions this script was written for
 'adbe': DeltaBox(prior_date='2026-08-17', prior_price=254.04, as_of='2026-09-01', price=286.08, state='price',
   claim='The price moved. Nothing else did.',
   paras=["Adobe has not reported since the previous edition. The rally tracked a broad enterprise-software "
   ...
   check="Re-verified: no earnings report (Q3 FY2026 is still ahead, on September 10), no guidance update, "
 ...
 'now': DeltaBox(prior_date='2026-08-17', prior_price=117.70, as_of='2026-09-01', price=142.90, state='price',
   claim='The price moved. Nothing else did.',
   paras=["No new operating results were published between the two editions. This was software multiple "
    ...
    "September 1."],
   check="Re-verified: no earnings report, no guidance update, no new disclosure. "
   "<b>Every fundamental figure below is unchanged from the August 17 edition; only the price-derived ones moved.</b>"),

 'stx': DeltaBox(prior_date='2026-08-17', prior_price=994.79, as_of='2026-09-01', price=816.64, state='price',
   claim='The price fell 17.9%. Seagate did nothing to cause it.',
   paras=["The decline was a sector-wide memory and storage de-rating, not a company event — contemporaneous "
   ...
   check="Re-verified: the demand picture, the HAMR position and the two-player structure in Sections 02, 03 "
   "and 06 were last set by the July 28 print and nothing since has touched them. "
   "<b>Only the price-derived figures moved.</b>"),

 'amat': DeltaBox(prior_date='2026-08-13', prior_price=548.15, as_of='2026-09-01', price=441.85, state='print',
   claim='New numbers from the Q3 FY2026 print. The direction is unchanged.',
   paras=["Applied reported Q3 FY2026 on August 13 — the same day as the previous edition — and beat on every "
   ...
   check="Updated at this edition: the income-statement figures and the year-to-date performance. "
 ...
 'crm': DeltaBox(prior_date='2026-08-24', prior_price=209.06, as_of='2026-09-01', price=258.11, state='print',
   claim='The business changed. This edition is substantially rewritten.',
   paras=["Salesforce reported Q2 FY2027 on August 26: revenue +10.8%, cRPO of $33.5B (+14% in constant "
   ...
   check="Superseded at this edition: the prior $209.06 price basis, the pre-print revenue and cRPO figures, "
 ...
 'gev': DeltaBox(prior_date='2026-08-12', prior_price=1039.90, as_of='2026-09-01', price=898.53, state='fix',
   claim='The price fell 13.6% on nothing — and this edition fixes a number the last one got wrong.',
   paras=["<b>The correction:</b> the previous edition carried a <em>modelled</em> FY2025 Electrification "
   ...
   check="Corrected: FY2025 Electrification segment revenue, now from the reported filing rather than a model. "
   "<b>Re-verified as unchanged:</b> capacity factors, guidance and signed PPAs — nothing broke operationally."),

 'klac': DeltaBox(prior_date='2026-08-12', prior_price=208.25, as_of='2026-09-01', price=170.89, state='fix',
   claim='The price fell 17.9% on a sector de-rating — and this edition replaces modelled segment data with reported figures.',
   paras=["<b>The correction:</b> the previous edition's segment splits were modelled. They have been replaced "
   ...
   check="Corrected: the Section 02 segment revenue splits, the China revenue share and the services share, "
   "all now from the 10-K. <b>Re-verified as unchanged:</b> the process-control moat and the competitive structure."),

 # ---- 16 Sep 2026 batch ----
 'nvda': DeltaBox(prior_date='2026-08-24', prior_price=208.48, as_of='2026-09-15', price=212.17, state='print',
   claim='NVIDIA reported Q2 FY2027 on August 26 — and the stock round-tripped the beat, closing September 15 just 1.8% above the prior edition.',
   paras=["<b>Q2 FY2027</b> (quarter ended July 26): revenue $96.2B (+106% YoY, +18% QoQ); Data Center $89.0B (+117%); "
   ...
   check="Updated at this edition: header, chart (the Aug 26 point is now the true month-end close), sections 01, 02, 04 "
 ...
 'hd': DeltaBox(prior_date='2026-08-03', prior_price=340.02, as_of='2026-09-15', price=305.48, state='print',
   claim='Home Depot beat and reaffirmed on August 18 — and the stock has fallen about 10% since, back to within 6% of its May low.',
   paras=["<b>Q2 FY2026</b> (reported August 18): sales $47.9B (+5.7%), comparable sales +1.7% (US +1.3%), comparable "
   ...
   check="Updated at this edition: header, banner, chart series and warnings, sections 01 through 08, industry commentary "
 ...
 'unh': DeltaBox(prior_date='2026-08-03', prior_price=416.83, as_of='2026-09-15', price=375.93, state='price',
   claim='UNH fell 9.8% between editions on no new company news — the multiple compressed, the business did not move.',
   paras=["Trailing P/E 26.7x → 24.2x, forward 19.5x → 17.5x, PEG 1.53 → 1.21, yield 2.23% → 2.47%, market cap $378.5B → "
   ...
   check="Updated at this edition: header, banner, chart, sections 01, 04, 05, 06, 07 and 08, industry commentary and "
...
def insert_box(path: str, e: DeltaBox) -> bool:
