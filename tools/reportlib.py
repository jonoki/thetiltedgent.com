"""How a published report page is read: its text and numbers, what the page says, and whether it is sound.
Imported by the scripts in tools/; not run on its own. Where the files are (the repo, report paths, the index
cards and the manifest's data files) is tools/repodata.py.
